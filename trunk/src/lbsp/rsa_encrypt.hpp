...
{
    RSA* prsa = RSA_new();
    LB_ADD_MEM(prsa, sizeof(RSA));
    BIO* bio = BIO_new_mem_buf(const_cast<char*>(pub_key.data()), static_cast<int>(pub_key.size()));
...
int private_key_decrypt(const char* ppriv_key, const char* penc_buf, int len, char* pdec_buf, int dec_buf_len, int padding_mode = RSA_PKCS1_PADDING)
{
    ...
    {
        srs_error("Invalid parameter, ppriv_key:%p || !penc_buf%p || !pdec_buf:%p, dec_buf_len:%d", ppriv_key, penc_buf, pdec_buf, dec_buf_len);
        return -1;
    }
    deinit();
    ...
    if(ret < 0)
    {
        srs_error("ret:%d = rsa_enc.init_private_key(ppriv_key) failed", ret);
        return ret;
    }
    ret = private_decrypt((const uint8_t*)penc_buf, len, (uint8_t*)pdec_buf, &dec_buf_len);
    if(ret < 0)
    {
        srs_error("ret:%d = rsa_enc.private_decrypt(penc_buf:%p, len:%d, decbuf:%p, &outlen:%d)", ret, penc_buf, len, pdec_buf, dec_buf_len);
        return ret;
    }
...
int private_key_encrypt(const char* ppriv_key, const char* porg_buf, int org_len, char* penc_buf, int enc_len, int padding_mode = RSA_PKCS1_PADDING)
{
    ...
    {
        srs_error("Invalid parameter, ppriv_key:%p || !porg_buf:%p || !penc_buf%p", ppriv_key, porg_buf, penc_buf);
        return -1;
    }
    deinit();
    int ret = init_private_key(ppriv_key, padding_mode);
    ...
    if(ret < 0)
    {
        srs_error("ret:%d = rsa_enc.private_decrypt(porg_buf:%p, org_len:%d, penc_buf:%p, &enc_len:%d)", ret, porg_buf, org_len, penc_buf, enc_len);
        return ret;
    }
...
int public_key_decrypt(const char* ppub_key, const char* penc_buf, int len, char* pdec_buf, int dec_buf_len, int padding_mode = RSA_PKCS1_PADDING)
{
    ...
    {
        srs_error("Invalid parameter, ppub_key:%p || !penc_buf%p || len:%d/128 != 0 || !pdec_buf:%p", ppub_key, penc_buf, len, dec_buf_len, pdec_buf);
        return -1;
    }
    deinit();
...
int public_key_encrypt(const char* ppub_key, const char* porg_buf, int org_len, char* penc_buf, int enc_buf_len, int padding_mode = RSA_PKCS1_PADDING)
{
    ...
    {
        srs_error("Invalid parameter, ppriv_key:%p || !porg_buf:%p || !penc_buf%p", ppub_key, porg_buf, penc_buf);
        return -1;
    }
    deinit();
    int ret = init_public_key(ppub_key, padding_mode);
    ...
    if(ret < 0)
    {
        srs_error("ret:%d = rsa_enc.private_decrypt(porg_buf:%p, org_len:%d, penc_buf:%p, &enc_buf_len:%d)", ret, porg_buf, org_len, penc_buf, enc_buf_len);
        return ret;
    }
