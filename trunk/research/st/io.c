...
int st_netfd_close(_st_netfd_t *fd)
{
    if ((*_st_eventsys->fd_close)(fd->osfd) < 0) {
	    printf("_st_eventsys->fd_close failed\n");
        num = -1;
        //return -1;
    }
...
{
    ssize_t n = 0;
    srs_trace("st_read timeout:%lld\n", timeout);
    while ((n = read(fd->osfd, buf, nbyte)) < 0) {
        if (errno == EINTR) {
            continue;
        }
        if (!_IO_NOT_READY_ERROR) {
	    srs_trace("st_read !_IO_NOT_READY_ERROR errno:%d\n", errno);
            return -1;
        }
        /* Wait until the socket becomes readable */
        if (st_netfd_poll(fd, POLLIN, timeout) < 0) {
	    srr_error("st_netfd_poll(fd, POLLIN, timeout) < 0\n");
            return -1;
        }
    }
...
{
    struct iovec iov, *riov = NULL;
    int riov_size = 0, rv = 0;
...
{
    ssize_t n = 0, rv = 0;
    size_t nleft = 0, nbyte = 0;
    int index = 0, iov_cnt = 0;
    struct iovec *tmp_iov = NULL;
    struct iovec local_iov[_LOCAL_MAXIOV];
    memset(&local_iov, 0, sizeof(iovec)*_LOCAL_MAXIOV);
