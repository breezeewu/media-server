...
# zlib
#SRS_OBJS_PATH=$(pwd -P)/objs
# zlib
#if [[ -f ${SRS_OBJS_PATH}/build/lib/libz.a ]]; then
#    echo "zlib is ok"
#else
#    echo "build zlib"
#    cd $SRS_OBJS_PATH && ls -al ../3rdparty/zlib-1.2.8.zip && ls -al &&
#    rm -rf zlib-1.2.8 && unzip -q ../3rdparty/zlib-1.2.8.zip -d ./ && ls -al &&
#    cd zlib-1.2.8 && ls -al ${SRS_OBJS_PATH} && chmod +x ./configure && ls -al && ./configure --prefix=${SRS_OBJS_PATH}/build && make ${SRS_OBJS_PATH}/build && make install
#    ret=$?; if [[ 0 -ne ${ret} ]]; then echo "build zlib-1.2.8 failed"; exit 1; fi
#    cd ${SRS_OBJS_PATH}
#fi
    ...
    if [ $SRS_USE_SYS_SSL = YES ]; then
        ...
        if [ $SRS_CROSS_BUILD = YES ]; then
            ...
            if [[ -f ${SRS_OBJS}/_flag.ssl.cross.build.tmp && -f ${SRS_OBJS}/openssl/lib/libssl.a ]]; then
                echo "openssl-1.1.0e for arm is ok.";
            else
                echo "build openssl-1.1.0e for arm"; 
                (
